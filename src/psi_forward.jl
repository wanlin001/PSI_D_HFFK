...
struct ForwardFiniteFrequency <: ForwardMethod
    ...
    n_rings::Int
    n_azimuth::Int
    tf_hffk_sp::Bool   # Apply HFFK Fresnel-smoothing to SplittingParameters (true) or fall back to waveform-only (false)
end
struct ParametersFiniteFrequency <: ForwardMethodParameters
    dominant_period::Float64
    n_rings::Int
    n_azimuth::Int
    tf_hffk_sp::Bool
...
function psi_forward(Observations::Vector{B}, Model::PsiModel{P}) where {B <: Observable, P}
    ...
    elseif isa(Observations[1].Forward, ForwardShortestPath)
        println("Starting 3D ray tracing...")
        predictions, relative_residuals, Kernels, _ = psi_forward_seismic_dijkstra(Observations, Model; length_0 = 0.0, pred = 0)
    elseif isa(Observations[1].Forward, ForwardFiniteFrequency)
        # HFFK: Heuristic Finite-Frequency slowness Kernel
        # VanderBeek & Faccenda (2021, GJI) eq.4
        println("HFFK forward: Fresnel zone weighted integration...")
        n = length(Observations)
        obs_type = typeof(Observations[1].observation)
        predictions = Vector{obs_type}(undef, n)
        relative_residuals = Vector{obs_type}(undef, n)
        Kernels = allocate_kernels_vector(Observations, Model)
        for i in eachindex(Observations)
            predictions[i], relative_residuals[i], Kernels[i] = psi_forward_hffk(Observations[i], Model)
        end
    else
...
function qs_phase_velocities_thomsen(propagation_azimuth, propagation_elevation, qt_polarization,
    ...
    g = 1.0 - ((β/α)^2)
    f = g^(-2)
    D = 0.5*g*( sqrt( max(0.0, 1.0 + 4.0*δ*f*sinθ_2*cosθ_2 + 4.0*ϵ*(g + ϵ)*f*(sinθ_2^2)) ) - 1.0 )
    # Exact qS-phase velocities (Thomsen, 1986; Eq. 10b,c)
    vqs1 = β*sqrt( max(0.0, 1.0 + ((α/β)^2)*(ϵ*sinθ_2 - D)) )
    vqs2 = β*sqrt( max(0.0, 1.0 + 2.0*γ*sinθ_2) )
...
# =============================================================================
# HFFK FORWARD: psi_forward_hffk
# VanderBeek & Faccenda (2021, GJI) eq.4
# =============================================================================

"""
    psi_forward_hffk(Observation, Model) -> (prediction, relative_residual, Kernel)

HFFK forward calculation for a single SplittingIntensity observation.
步驟:
  1. 用 TauP 取得射線路徑（同 ray theory）
  2. 沿射線每個取樣點，計算 Fresnel zone 半徑 R_f(x)
  3. 在 R_f 截面內取 n_rings × n_azimuth 個偏移點
  4. 對每個偏移點內插模型參數，以 HFFK weight 加權
  5. 積分得 splitting intensity 預測值
"""
# SplittingParameters + HFFK
# tf_hffk_sp=true  → Fresnel-smooth the Cij at each ray point, then run waveform propagation (banana-donut effect)
# tf_hffk_sp=false → pure waveform propagation on the geometric ray (same as ray theory)
function psi_forward_hffk(Observation::SplittingParameters, Model::PsiModel)
    Kernel = return_kernel(Observation, Model)
    if !Observation.Forward.tf_hffk_sp
        prediction, relative_residual = evaluate_kernel(Kernel)
        return prediction, relative_residual, Kernel
    end
    _hffk_smooth_cij!(Kernel, Observation, Model)
    prediction, relative_residual = evaluate_kernel(Kernel)
    return prediction, relative_residual, Kernel
end

# _hffk_smooth_cij! is defined in psi_forward_elastic_tensor.jl (ElasticVoigt type is defined there)

function psi_forward_hffk(Observation::SplittingIntensity, Model::PsiModel)
    # --- Step 1: 建立射線路徑（與 TauP 相同）---
    Kernel = return_kernel(Observation, Model)

    # --- Step 2–5: HFFK 積分 ---
    prediction = Kernel.static[1]
    n_rays = length(Kernel.weights)

    # 射線總長度 (km)
    L = sum(w.dr for w in Kernel.weights)

    # 從 Observation.Forward 取得 HFFK 參數
    period    = Observation.Forward.dominant_period   # dominant period (s)
    n_rings   = Observation.Forward.n_rings           # 徑向環數
    n_azimuth = Observation.Forward.n_azimuth         # 每環方位數
    ...
    for (i, w) in enumerate(Kernel.weights)
        dr = w.dr
        x_mid = x_cumulative + dr / 2.0   # 此段中點的沿射線距離

        # 1D 參考 S-wave slowness（用模型在此點的 vs）
        vs_ref = kernel_phase_velocity(Observation.Phase, Kernel, i)
        u_ref  = 1.0 / max(vs_ref, 0.1)   # s/km，避免除以零

        # Fresnel zone 半徑
        R_f = fresnel_radius(x_mid, L, period, u_ref)

        # 若 R_f 很小（射線端點或 TauP 解析失敗），直接退化為 ray theory
        if R_f < 0.5
            si_ray = kernel_splitting_intensity(Observation.Phase, Kernel, i)
            prediction += dr * si_ray
            x_cumulative += dr
            continue
        end
        ...
        # 射線方位角與仰角（用 kernel weights 中的幾何資訊）
        azimuth_ray   = w.azimuth
        elevation_ray = w.elevation

        # 在 Fresnel zone 截面內取樣
        offsets = fresnel_sample_offsets(R_f, n_rings, n_azimuth)

        # 加權累積 splitting intensity
        si_weighted = 0.0
        total_weight = 0.0

        for (Δe, Δn, hffk_w) in offsets
            if Δe == 0.0 && Δn == 0.0
                # 中心點退化為 ray theory（HFFK sin→0，但保留以維持中心貢獻）
                si_pt = kernel_splitting_intensity(Observation.Phase, Kernel, i)
                # 中心點 HFFK 貢獻極小（sin(0)=0），實際可略；但加入保持完整
                # weight = hffk_weight(0, R_f) = 0，所以不影響結果
                continue
            end
            ...
            si_pt = interpolate_si_at_point(Observation.Phase, Kernel, i,
                                             qx_off_local, Model, w)

            si_weighted  += hffk_w * si_pt
            total_weight += hffk_w
        end

        # 正規化（total_weight ≈ ∫K dA over discrete samples）
        si_final = total_weight > 0.0 ? si_weighted / total_weight : 0.0
        prediction += dr * si_final

        x_cumulative += dr
    end

    relative_residual = (Observation.observation - prediction) / Observation.error
    return prediction, relative_residual, Kernel
end

"""
    interpolate_si_at_point(Phase, Kernel, ray_idx, qx_local, Model, w)

在偏移座標 qx_local 插值模型參數，計算 splitting intensity。
利用 trilinear_weights + HexagonalVectoralVelocity 參數。
"""
function interpolate_si_at_point(Phase::ShearWave,
                                  Kernel::ObservableKernel{<:SplittingIntensity, <:HexagonalVectoralVelocity, <:Any, <:Any, <:Any},
                                  ray_idx::Int,
                                  qx_local,
                                  Model::PsiModel{<:HexagonalVectoralVelocity},
                                  w)
    wind, wval = trilinear_weights(Model.Mesh.x, qx_local; tf_extrapolate = true, scale = 1.0)

    # 從三線性插值取得平均模型參數
    α_pt = 0.0; β_pt = 0.0; f_pt = 0.0
    ratio_ϵ_pt = 0.0; ratio_η_pt = 0.0; ratio_γ_pt = 0.0
    s1 = 0.0; s2 = 0.0; s3 = 0.0

    # 判斷 ratio 是否為純量（固定值）還是向量
    ratio_is_scalar = Model.Parameters.ratio_ϵ isa Number

    for (j, wj) in enumerate(wval)
        k = wind[j]
        α_pt += wj * Model.Parameters.α[k]
        β_pt += wj * Model.Parameters.β[k]
        f_pt += wj * Model.Parameters.f[k]
        sinϕ, cosϕ = sincos(Model.Parameters.azimuth[k])
        sinθ, cosθ = sincos(Model.Parameters.elevation[k])
        s1 += wj * cosθ * cosϕ
        s2 += wj * cosθ * sinϕ
        s3 += wj * sinθ
        if !ratio_is_scalar
            idx_r = min(k, length(Model.Parameters.ratio_ϵ))
            ratio_ϵ_pt += wj * Model.Parameters.ratio_ϵ[idx_r]
            ratio_η_pt += wj * Model.Parameters.ratio_η[idx_r]
            ratio_γ_pt += wj * Model.Parameters.ratio_γ[idx_r]
        end
    end
    if ratio_is_scalar
        ratio_ϵ_pt = Model.Parameters.ratio_ϵ
        ratio_η_pt = Model.Parameters.ratio_η
        ratio_γ_pt = Model.Parameters.ratio_γ
    end

    # Thomsen parameters（與 return_thomsen_parameters 相同定義：δ = ϵ - η）
    ϵ_pt = f_pt * ratio_ϵ_pt
    η_pt = f_pt * ratio_η_pt
    γ_pt = f_pt * ratio_γ_pt
    δ_pt = ϵ_pt - η_pt

    # 恢復對稱軸方位角與仰角
    snorm = sqrt(s1^2 + s2^2 + s3^2)
    if snorm < 1e-8
        return 0.0
    end
    az_pt = atan(s2, s1)
    el_pt = asin(clamp(s3 / snorm, -1.0, 1.0))

    vqs1_pt, vqs2_pt, _, ζ_pt = qs_phase_velocities_thomsen(
        w.azimuth, w.elevation, Phase.paz,
        az_pt, el_pt, α_pt, β_pt, ϵ_pt, δ_pt, γ_pt)

    # Guard against unphysical velocities from out-of-domain extrapolation
    (vqs1_pt <= 0.0 || vqs2_pt <= 0.0) && return 0.0

    # 與 kernel_splitting_intensity 相同公式（sin 非 cos）
    si_pt = 0.5 * (1.0/vqs2_pt - 1.0/vqs1_pt) * sin(2.0 * ζ_pt)

    return si_pt
end

# interpolate_si_at_point for ElasticVoigt is defined in psi_forward_elastic_tensor.jl
# (ElasticVoigt type is defined there, so the method must live there too)
