...
function build_inputs(param_file)
    ...
    # HFFK: allows TauP + FiniteFrequency together (TauP still needed for ray path in HFFK)
    if !haskey(D["Model"]["Methods"], "FiniteFrequency")
        length(D["Model"]["Methods"]) > 1 && error("Multiple forward methods not supported!")
    end
    if haskey(D["Model"]["Methods"], "TauP") && !haskey(D["Model"]["Methods"], "FiniteFrequency")
        FwdType = ForwardTauP
        FwdInstance = ForwardTauP()
    elseif haskey(D["Model"]["Methods"], "ShortestPath")
        FwdType = ForwardShortestPath
        FwdInstance = ForwardShortestPath()
    elseif haskey(D["Model"]["Methods"], "FiniteFrequency")
        ...
        FwdInstance = ForwardFiniteFrequency(
            ...
            get(ff, "n_rings", 3),
            get(ff, "n_azimuth", 8),
            get(ff, "tf_hffk_sp", true)
        )
        FwdType = ForwardFiniteFrequency
    else
        error("Could not identify forward method!")
    end
    Obs = build_observations(D["Observations"]; FwdType = FwdType, FwdInstance = FwdInstance)
...
function build_observations(D::Dict; FwdType = ForwardTauP, FwdInstance = ForwardTauP(), dlm = ",", ValueType = Float64)
    ...
    for bkey in eachindex(D) # Loop over SeismicObservable types
        ...
        for pkey in eachindex(D[bkey]) # Loop over SeismicPhase types
            ...
            for line in readlines(D[bkey][pkey]["filename"]) # Loop over observations
                ...
                bvec[iobs] = ObsType(aPhase, FwdInstance, sid, rid, b, σ)
            end
        end
    end
...
function read_model(::Type{LocalGeographic}, ::Type{RegularGrid}, parameterisation, f; dlm = ",", T = Float64,
    ...
    dz_ext = (length(line) > 3 && !isempty(strip(line[4]))) ? parse(T, strip(line[4])) : 0.0 # Read starting depth for vertically extended models
...
function read_model_parameters(io, parameterisation::Type{HexagonalVectoralVelocity}, Mesh; dlm = ",", T = Float64)
    ...
    nxyz = size(Mesh)
    line = readline(io)
    line = filter(!isempty, strip.(split(line, dlm)))
    if length(line) == 1
        tf_read_ratios = true
